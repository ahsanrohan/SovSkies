...
class PlayerPlaneHandler():
    def __init__(self,ownedPlanes, *args, **kwargs):
        ...
        helicopter_animation = pyglet.image.Animation.from_image_sequence(helicopter, duration=0.1, loop=True)
        for plane in ownedPlanes:
            if plane[0] == "fast_plane":
                self.planes.append(PlayerPlane("fast_plane", 2000, 30, plane_2, 1, 0.05, 10, "laser",  [0], **kwargs))
            if plane[0] == "damage_plane":
                self.planes.append(PlayerPlane("damage_plane", 1200, 50, plane_1, 2, 0.05, 10, "fire_rate_increase",  [50, -50], **kwargs))
            if plane[0] == "helicopter":
                self.planes.append(PlayerPlane("helicopter", 1600, 40, helicopter_animation, 3, 0.05, 40, "ramming", [0], **kwargs))
            if plane[0] == "support_plane":
                self.planes.append(PlayerPlane("support_plane", 1200, 80, enemy_plane_2, 4, 0.1, 20, "revive", [0], **kwargs))
